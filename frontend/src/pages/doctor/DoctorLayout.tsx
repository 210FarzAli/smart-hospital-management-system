        ...
        <div className="border-t border-teal-900/80 p-4">
          <div className="mb-3 flex items-center gap-3 rounded-xl bg-teal-900/40 p-2.5">
            <div className="flex h-9 w-9 shrink-0 items-center justify-center rounded-lg bg-teal-800 text-xs font-bold text-white">
              {staffUser?.fullName
                ? staffUser.fullName.replace(/^Dr\.\s*/i, "").charAt(0).toUpperCase()
                : "D"}
            </div>
            <div className="min-w-0 flex-1">
              <div className="truncate text-xs font-bold text-white">
                {staffUser?.fullName || "Physician"}
              </div>
        ...
        <header className="flex h-16 shrink-0 items-center justify-between border-b border-slate-200 bg-white px-4 sm:px-8 shadow-xs">
          <div className="flex items-center gap-3">
            ...
            <div className="hidden items-center gap-2 text-xs font-medium text-slate-500 sm:flex">
              ...
              <span>/</span>
              <span className="font-semibold text-teal-950">Clinical Management</span>
            </div>
          </div>

          <div className="flex items-center gap-4">
            <div className="flex items-center gap-2 rounded-full bg-teal-50 px-3 py-1 text-xs font-semibold text-teal-800 ring-1 ring-teal-200">
              <Stethoscope className="h-3.5 w-3.5" />
              <span>OPD Shift Active</span>
            </div>

            <div className="hidden text-right text-xs sm:block">
              <div className="font-bold text-teal-950">{staffUser?.fullName}</div>
              <div className="text-slate-400">{staffUser?.email || "physician@hospital.com"}</div>
            </div>
          </div>
        </header>
